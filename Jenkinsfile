pipeline {
    ...
    tools {
        jdk 'JDK21'        // Configure name in Jenkins -> Global Tool Configuration
        maven 'Maven3'     // Configure Maven installation in Jenkins
    }

    environment {
        TOMCAT_HOME = "/usr/local/Cellar/tomcat/11.0.10/libexec"   // Change this to your Tomcat installation path
    }
    ...
    stages {
        ...
        stage('Debug Path') {
            steps {
                sh 'ls -ld /usr/local/Cellar/tomcat/11.0.10/libexec/webapps'
